...
import uvicorn
import logging
import httpx  # New import needed for async HTTP requests
...
async def predict(
    ...
    """Perform inference on an image and text pair.
       Accepts an image file upload or an image URL.
    """
    # Retrieve image bytes either from file or URL
    if image_url:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise HTTPException(
                    status_code=400,
                    detail="Failed to retrieve image from provided URL."
                )
        ...
        raise HTTPException(
            status_code=400,
            detail="No image provided. Please upload an image file or provide an image URL."
        )
    ...
    image_tensor = transform(image_obj).unsqueeze(0).to(device)  # Add batch dimension
    ...
    # Perform inference
    with torch.no_grad():
        output = model(image_tensor, input_ids, attention_mask)  # Forward pass
